<!-- Typing Intro -->

Hello! 👋 My name is **Yuvarrunjitha R S**. I am from Tamil Nadu, India 🇮🇳 and currently pursuing **B.E in Computer Science and Engineering (Artificial Intelligence & Machine Learning)** at **Chennai Institute of Technology** 🎓.

I have a huge interest in **Artificial Intelligence, Machine Learning, Software Development, and Research** 🤖💡. I enjoy building real-world solutions using **AI Agents, RAG systems, Full Stack Development, and Cloud technologies** 🚀. Currently exploring advanced **LLMs, System Design, and Open Source**.

Currently an **IEEE Published Research Author** 📄, **GDG On Campus Co-Lead** 📱, and an active participant in hackathons and developer communities 🏆. I love collaborating with teams and building technology that creates real impact for society ✨.

### Skills:
Artificial Intelligence | Machine Learning | Full Stack Development | Problem Solving | Research | Community Building  

### Tools:
`Python` `Java` `C++` `React.js` `Node.js` `MongoDB` `MySQL` `RAG` `LLMs` `n8n`

- 🌱 I’m currently learning **Advanced AI Agents & System Design**
- 🚀 I’m currently building **AI-powered products and real-world applications**
- 📫 How to reach me: **yuvarrunjithars@gmail.com**
- 😄 Pronouns: **She/Her**
- ⚡ Fun fact: I love rainy vibes 🌧, chocolate 🍫, and turning ideas into products ✨

### 🚀 Featured Work

- [AskMyDocs - RAG-powered document Q&A](https://github.com/2024yuva/AskMyDocs)
- [AI Payment Reminder Agent  ](https://github.com/AI-Developing-Hunters/AI-Payment-Reminder-Agent)
- [Smart Detect - Real-time IDS](https://github.com/2024yuva/Intrusion-Detection-System)

---

  <!-- SSOC -->
  <img src="https://img.shields.io/badge/SSOC'25-Open%20Source%20Contributor-008000?style=for-the-badge&logo=opensourceinitiative&logoColor=white" />

  <!-- GSSoC -->
  <img src="https://img.shields.io/badge/GSSoC'25-Contributor-FB8C00?style=for-the-badge&logo=github&logoColor=white" />


### 🌐 Connect

[LinkedIn](https://linkedin.com/in/yuvarrunjitha-r-s)
[GitHub](https://github.com/2024yuva)
[Email](mailto:yuvarrunjithars@gmail.com)
[Discord](y_slayz)

---
> *Code. Build. Research. Repeat.*
<!---
2024yuva/2024yuva is a ✨ special ✨ repository because its `README.md` appears on your GitHub profile.
You can click the Preview link to take a look at your changes.
--->

